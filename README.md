...
Windows 下双击项目根目录的 `打开最新体验.cmd`。它会自动检查并恢复由项目托管的本地预览服务，然后打开固定地址 `http://127.0.0.1:4173/`；每次打开都会附加刷新标记，避免浏览器继续显示旧页面。开发版还会在同源窗口之间同步预览会话：服务重启后，仍打开的旧标签会自动重新载入。后台服务的日志保存在 `.local-preview/`，电脑重启后再次双击即可恢复。日常评审以该地址为准，`dist/index.html` 仅用于离线成品验证。
